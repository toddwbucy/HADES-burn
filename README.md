...
**Semantic code ingestion.** Rust uses `syn` plus rust-analyzer, Python uses `rustpython-parser`, and C, C++, and CUDA use libclang. C-family ingestion understands namespaces, templates, overloads, calls, and CUDA kernel launches; it can consume a project's `compile_commands.json`. Chunking respects symbol boundaries, and cross-file relationships produce typed graph edges. Embedding is optional and decoupled from structural ingestion.
